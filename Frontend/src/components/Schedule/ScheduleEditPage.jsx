...
export default function ScheduleEditPage() {
  ...
  const load = async () => {
    try {
      const [groupRes, scheduleRes, slotsRes, sgRes, subjectsRes] =
        await Promise.all([
          getGroupById(id),
          getScheduleByGroup(id),
          getLessonSlots(),
          getSubGroups(id),
          getSubjectsByGroup(id),
        ]);
  ...
  const handleSgSave = async () => {
    ...
    try {
      if (sgEditTarget) {
        ...
        setSubGroups((prev) =>
          prev.map((sg) => (sg.id === sgEditTarget.id ? res.data : sg)),
        );
      } else {
  ...
  const handleSgDelete = async (sg) => {
    if (!confirm('Видалити підгрупу "' + sg.name + '"?')) {
      return;
    }
    try {
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      >
        <button
          ...
          style={{
            ...
            border: 'none',
            borderRadius: '8px',
            cursor: slots.length >= 7 ? 'default' : 'pointer',
            fontWeight: 600,
          }}
        >
          {seeding
            ...
            : slots.length >= 7
              ? '✅ 7 пар вже є в БД'
              : '⚡ Автозаповнити 7 пар у БД'}
        </button>
        {seedMsg && (
          <span style={{ color: '#28a745', fontWeight: 500 }}>{seedMsg}</span>
        )}
        <button
      ...
      {slots.length === 0 && (
        ...
        >
          ⚠️ У БД немає пар. Натисніть{' '}
          <strong>«Автозаповнити 7 пар у БД»</strong>.
        </div>
      )}
      ...
      {sgModalOpen && (
        ...
        >
          ...
          >
            <h3 style={{ margin: 0 }}>
              {sgEditTarget ? 'Редагувати підгрупу' : 'Нова підгрупа'}
            </h3>
            <input
              placeholder="Назва (напр. 344(1))"
              value={sgName}
