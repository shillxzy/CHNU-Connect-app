...
// =====================================================
// Конвертери enum-рядків → числа (бо бекенд повертає
// "Monday", "First", "Practice" через JsonStringEnumConverter)
// =====================================================
...
function normalizeSchedule(raw) {
  return raw.map((item) => ({
    ...item,
    // Якщо прийшов рядок — конвертуємо, якщо вже число — лишаємо
...
export default function ScheduleEditPage() {
  ...
  const load = async () => {
    try {
      const [groupRes, scheduleRes, slotsRes, sgRes] = await Promise.all([
        getGroupById(id),
        getScheduleByGroup(id),
        getLessonSlots(),
        getSubGroups(id),
      ]);
      setGroup(groupRes.data);
      setSchedule(normalizeSchedule(scheduleRes.data || [])); // ✅ нормалізація
  ...
  const handleSaveLesson = async (lesson) => {
    try {
      const dotNetDay = Number(lesson.day) + 1; // grid 0=Пн → .NET Monday=1
      const payload = {
        groupId:     Number(id),
      ...
      // ✅ нормалізуємо і нову запис перед додаванням у стейт
  ...
  const handleMove = async (lesson) => {
    try {
      ...
      const payload = {
        ...
        newDay:     dotNetDay,
        newSlotId:  Number(lesson.slotId),
        newWeek:    Number(lesson.week),
      };
      const res = await moveSchedule(payload);
      if (!res.data) {return;}
      setSchedule((prev) =>
        prev.map((l) =>
          l.id === lesson.id
            ? { ...l, day: dotNetDay, slotId: lesson.slotId, week: lesson.week }
            : l,
        ),
      );
    } catch (err) {
    ...
    <div style={{ padding: '20px' }}>
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '16px', flexWrap: 'wrap', alignItems: 'center' }}>
        <button
          ...
          disabled={seeding || slots.length >= 7}
          style={{
            padding: '8px 16px',
            background: slots.length >= 7 ? '#6c757d' : '#007bff',
            color: 'white', border: 'none', borderRadius: '8px',
            cursor: slots.length >= 7 ? 'default' : 'pointer', fontWeight: 600,
          }}
        >
        ...
        <div style={{ marginBottom: '16px', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          {subGroups.map((sg) => (
            <div key={sg.id} style={{
              ...
              background: '#e9ecef', borderRadius: '20px', padding: '4px 12px', fontSize: '14px',
            }}>
              <span>{sg.name}</span>
              <button onClick={() => openSgEdit(sg)}
                style={{ background: 'none', border: 'none', cursor: 'pointer' }} title="Редагувати">✏️</button>
              <button onClick={() => handleSgDelete(sg)}
                style={{ background: 'none', border: 'none', cursor: 'pointer' }} title="Видалити">🗑️</button>
            </div>
          ))}
        </div>
      )}

      {/* Попередження якщо немає слотів */}
      {slots.length === 0 && (
        <div style={{
          padding: '12px 16px', background: '#fff3cd',
          border: '1px solid #ffc107', borderRadius: '8px',
          marginBottom: '16px', color: '#856404',
        }}>
